...
The MCP server exposes the requested portfolio, balance, position, order, market, indicator, bot, performance, history, and risk tools at `POST /mcp/{workspaceHandle}` (Streamable HTTP). The unique workspace handle is the only credential — no bearer token is required. All MCP mutations use the same command application services and risk checks as REST.
