...
Controls cover CSRF, strict CSP and security headers, output encoding, schema validation, parameterized database access, CORS allowlists, SSRF-safe outbound destinations, request/body limits, IP and account rate limiting, webhook HMAC with nonce replay locks, RBAC, tenant-isolation tests, secret scanning, dependency/SAST/container scanning, and append-only audit records. The MCP server is tenant-scoped by its unique workspace handle in the URL; all tool calls still pass the same risk engine and workspace isolation checks as REST.
